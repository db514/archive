...
namespace WpfRandomSelector
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void BtnSetParentFolder_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                folderBrowserDialog.InitialDirectory = LblParentFolder.Content.ToString();
            }
            if (folderBrowserDialog.ShowDialog() == true)
            {
                LblParentFolder.Content = folderBrowserDialog.InitialDirectory;
                folderSelector.GetItemList(folderBrowserDialog.InitialDirectory);
                MainWindow mainWindow = this;
                mainWindow.fileSelector.GetItemList(folderBrowserDialog.InitialDirectory);
                mainWindow.CmbFolderList.Items.Add(folderBrowserDialog.InitialDirectory);
        ...
        private void ManuallyLoadDirectory_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (folderBrowserDialog.ShowDialog() == true)
            {
                ...
                LblSelectedItem.Content = folderBrowserDialog.InitialDirectory;
                folderSelector.CurrentSelectedItem = folderBrowserDialog.InitialDirectory;

                Cursor = System.Windows.Input.Cursors.Wait;

                LblParentFolder.Content = Path.GetDirectoryName(folderBrowserDialog.InitialDirectory);

                folderSelector.RandomlyCopyImageFiles(folderBrowserDialog.InitialDirectory);
        ...
        private void ClosePreviouslyOpenedExplorerWindows(string path, string alternatePath = "")
        {
            ...
            void CloseWindowsExplorer(string path)
            {
                IntPtr i = FindWindowByCaption(IntPtr.Zero, path);
                if (i.Equals(IntPtr.Zero) == false)
                {
                    // WM_CLOSE is 0x0010
                    IntPtr result = SendMessage(i, 0x0010, IntPtr.Zero, null);
                }
            }
